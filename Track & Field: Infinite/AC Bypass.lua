--[[
	...
	@DownInDaNang - fyi ws resets after 1 sec, i disabled the kick but cant stop the actual reset loop yet, if u find a way hmu on discord
        ...
        if table.find(consts, "Hooked Env") then
            for idx, const in consts do
                if const == "ReplicatedFirst.Animate" then
                    debug.setconstant(v, idx, debug.info(v, "s"))
                end
            end
        end
